...
class FormUpdateAnnonceChecker extends FormChecker {

    public function check($nAnnonceId, CategoriesModel $oCategoriesModel, AnnoncesModel $oAnnoncesModel, AnnoncesPicturesModel $oAnnoncesPicturesModel) {
        ...
        $nCountPicture = $oAnnoncesPicturesModel->getCountFromAnnonce($nAnnonceId);
        $nMinPictures = max(Constants::ANNONCE_PICTURES_MIN - $nCountPicture, 0);
        $nMaxPictures = Constants::ANNONCE_PICTURES_MAX - $nCountPicture;
        $this->checkFiles("pictures", $nMinPictures, $nMaxPictures, Constants::ANNONCE_PICTURE_MAX_OCTET, Constants::ANNONCE_PICTURES_ACCEPT_EXT, true);
        ...
        if ($this->getSuccess()) {
            ...
            $aValues = [$_POST["category"], $_POST["title"], $nPrice, $_POST["description"], 0];

            try {
                $oAnnoncesModel->modify($nAnnonceId, $aValues);
                $this->uploadFiles("pictures", $nAnnonceId, $oAnnoncesPicturesModel);
                $this->pushSuccess("global", "Votre annonce à bien été modifiée");
            } catch (Exception $e) {
